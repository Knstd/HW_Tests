from Task_1.data import documents, directories
...
def remove_doc_from_shelf(user_doc_number, directories):
...
def get_doc_shelf(user_doc_number, documents, directories):
    doc_exist = check_document_existance(user_doc_number, documents)
    if doc_exist:
        for directory_number, directory_docs_list in directories.items():
            if user_doc_number in directory_docs_list:
...
def secretary_program_start():
    ...
    while True:
        ...
        if user_command == 'p':
            ...
            user_doc_number = input('Введите номер документа - ')
            directory_number = get_doc_shelf(user_doc_number, documents, directories)
            ...
            user_doc_number = input('Введите номер документа - ')
            doc_number, deleted = delete_doc(user_doc_number, documents, directories)
            if deleted:
